...
export default function LiveAuctionPage() {
  ...
  const [bids, setBids] = useState(auctionData.initialBids);
    ...
    <main className="flex-1 flex flex-col min-h-screen bg-[#F4F5F7] dark:bg-[#0A101C] pt-32 sm:pt-36 pb-16 transition-colors">
      ...
      <div className="w-full max-w-7xl mx-auto px-6 lg:px-12 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 flex flex-col gap-8">
          ...
          {/* Gallery */}
          <div className="bg-white dark:bg-[#1E293B] p-2 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-800">
            <div className="relative w-full aspect-[4/3] sm:aspect-[16/9] rounded-2xl overflow-hidden mb-2 bg-gray-100 dark:bg-slate-900">
          ...
          {/* Property Info */}
          <div className="bg-white dark:bg-[#1E293B] rounded-3xl p-6 sm:p-8 shadow-sm border border-gray-100 dark:border-slate-800">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 py-6 border-y border-gray-100 dark:border-slate-700 mb-8">
        ...
        {/* Right Column: Bidding Dashboard (4 cols) */}
          ...
          <div className="sticky top-24 flex flex-col gap-6">
            ...
            <div className="bg-[#1B3A2D] dark:bg-[#0A1612] rounded-3xl p-6 shadow-2xl relative overflow-hidden">
              ...
              <div className="relative z-10 flex flex-col">
                <p className="text-white/60 text-[13px] font-bold uppercase tracking-widest mb-2">{content.bidding.endsIn}</p>
                ...
                <div className="bg-white/5 border border-white/10 rounded-2xl p-5 mb-8 backdrop-blur-md">
                  <p className="text-[#5CD284] text-[12px] font-bold uppercase tracking-widest mb-1">{content.bidding.currentBid}</p>
                ...
                <form onSubmit={handlePlaceBid} className="flex flex-col gap-4">
                  <button type="submit" className="w-full bg-[#5CD284] hover:bg-[#4ab872] text-[#0A1C12] font-bold text-[16px] py-4 rounded-xl transition-all duration-300 shadow-[0_0_20px_rgba(92,210,132,0.3)] hover:shadow-[0_0_30px_rgba(92,210,132,0.5)]">
                    {content.bidding.placeBidBtn} (Ð {(currentBid + auctionData.minIncrement).toLocaleString()})
                  </button>
                  <p className="text-white/40 text-[11px] text-center">
                    By placing a bid, you agree to our Terms and Conditions. A 10% deposit is required upon winning.
                  </p>
                </form>
              </div>
            </div>
            ...
            {/* Bid History */}
            <div className="bg-white dark:bg-[#1E293B] rounded-3xl p-6 shadow-sm border border-gray-100 dark:border-slate-800">
              <div className="flex items-center justify-between mb-6">
                <h3 className="font-bold text-[18px] text-gray-900 dark:text-white">Live Bids</h3>
                <span className="bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-gray-300 text-[12px] font-bold px-3 py-1 rounded-lg">
                  {bids.length} Bids
                </span>
              </div>
              ...
              <div className="flex flex-col gap-4 overflow-y-auto global-green-scrollbar pr-2 max-h-[300px]">
                {bids.map((bid: any, i: number) => (
                  <div key={bid.id} className="flex items-center justify-between p-4 rounded-xl bg-gray-50 dark:bg-slate-800/50 border border-gray-100 dark:border-slate-700">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-gray-200 dark:bg-slate-700 flex items-center justify-center text-[14px] font-bold text-gray-600 dark:text-gray-300">
                        {bid.user[0]}
                      </div>
                      <div className="flex flex-col">
                        <span className="font-bold text-[14px] text-gray-900 dark:text-white">{bid.user}</span>
                        <span className="text-[12px] text-gray-500 dark:text-gray-400">{bid.time}</span>
                      </div>
                    </div>
                    <div className="flex flex-col items-end">
                      <span className="font-bold text-[15px] text-[#1A3626] dark:text-[#5CD284]">
                        Ð {bid.amount.toLocaleString()}
                      </span>
                      {i === 0 && (
