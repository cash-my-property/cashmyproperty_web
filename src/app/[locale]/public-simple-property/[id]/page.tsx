...
async function getPropertyData(id: string) {
  try {
    ...
    const res = await fetch(`${API_URL}/public/simple-property-details/${id}`, {
    ...
    console.error("Error fetching simple listing data on server:", err);
    return null;
  }
}
...
export async function generateMetadata(props: { params: Promise<{ id: string; locale: string }> }): Promise<Metadata> {
  ...
  if (!propertyInfo) {
    return {
      title: "Property Not Found | Cash My Property",
...
export default async function PublicSimplePropertyDetailPage(props: { params: Promise<{ id: string; locale: string }> }) {
  const params = await props.params;
