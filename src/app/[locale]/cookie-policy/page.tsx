...
export default function CookiePolicyPage() {
  ...
  // Safely fetch cookiePolicy and terms dictionaries
  const cookiePolicy = (dict.cookiePolicy || {}) as Record<string, any>;
  const terms = (dict.terms || {}) as Record<string, any>;

  const pageTitle = cookiePolicy.title || "Cookie Policy";
  const lastUpdatedLabel = cookiePolicy.lastUpdated || "Last Updated";
  const intro1 = cookiePolicy.intro1 || "";
  const intro2 = cookiePolicy.intro2 || "";

  // Parse sections dynamically from localized translation dictionaries
  const sections = useMemo(() => {
    const sectionMap = new Map<number, { title: string; items: CookieItem[] }>();
    const keys = Object.keys(cookiePolicy);

    for (const key of keys) {
      const match = key.match(/^s(\d+)(.*)/);
      if (match) {
        const secNum = parseInt(match[1], 10);
        const suffix = match[2]; // e.g. "Title", "aTitle", "aDesc", etc.
        const val = cookiePolicy[key] as string;
  ...
  const renderItem = (item: CookieItem, key: string) => {
