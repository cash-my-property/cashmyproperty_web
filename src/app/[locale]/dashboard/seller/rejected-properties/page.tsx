...
export default function RejectedPropertiesPage() {
  ...
  // Redirect Simple Sellers to simple rejected page
  useEffect(() => {
    if (authLoading || !user) return;
    if (isSeller && sellerType === 'SIMPLE') {
      router.replace(`/${locale}/dashboard/seller/rejected-simple-properties`);
    }
  ...
  const fetchProperties = async (pageNum: number = 1, append: boolean = false) => {
    ...
    try {
      ...
      let url = `/seller/rejectedProperties?page=${pageNum}&limit=10&sortBy=${sortBy}`;
      if (statusFilter !== "all") {
        url += `&status=${statusFilter}`;
      }
      const response = await api.get(url);
      const resData = response.data?.result || response.data;
      const newItems = resData?.listings || resData?.data || [];
  ...
  if (sellerType !== 'REGULAR') {
    ...
    return (
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 p-4 sm:p-8">
        ...
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-3xl p-8 flex flex-col items-center text-center shadow-sm max-w-xl mx-auto mt-8">
          <AlertCircle className="w-12 h-12 text-amber-600 dark:text-amber-500 mb-4" />
          <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Switch to Realtime Agent Mode</h3>
          <p className="text-gray-500 dark:text-gray-400 mb-6 max-w-md">
            You are currently in Listing Agent mode. Please switch to Realtime Agent mode to view your rejected regular properties (Realtime Offers).
          </p>
          <button
            onClick={async () => {
              try {
